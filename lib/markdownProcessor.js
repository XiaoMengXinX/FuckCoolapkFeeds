...
import { processCoolapkEmojiWithCodeProtection } from './emojiProcessor';
 ...
 * 支持服务端和客户端环境
 */
export const decodeEntities = (text, enableEmoji = true) => {
    // 傻逼酷安乱解析链接
    text = text.replace(/<a class="feed-link-url" href=["']([^"']+)["'][^>]*>.*?<\/a>/g,
        (match, url) => convertCoolapkFeedLinks(url)
    );
    
    // 根据参数决定是否处理酷安表情
    if (enableEmoji) {
        text = processCoolapkEmojiWithCodeProtection(text);
    }
    ...
    const mdInstance = new MarkdownIt({
        ...
        highlight: function (str, lang) {
            ...
            if (lang && hljs.getLanguage(lang)) {
                try {
                    return '<pre data-language="' + mdInstance.utils.escapeHtml(lang) + '"><code class="hljs">' +
                        hljs.highlight(cleanedStr, { language: lang, ignoreIllegals: true }).value +
                        '</code></pre>';
                } catch (__) { }
            }
            const langAttr = lang ? ' data-language="' + mdInstance.utils.escapeHtml(lang) + '"' : '';
            return '<pre' + langAttr + '><code class="hljs">' + mdInstance.utils.escapeHtml(cleanedStr) + '</code></pre>';
        }
    })
