...
// 酷安表情CDN基础URL
const EMOJI_BASE_URL = "http://static.coolapk.com/emoticons/v9/";

/**
 * 将表情名转换为完整的图片URL
 */
const getEmojiUrl = (emojiName) => {
    const filename = COOLAPK_EMOJI_MAP[emojiName];
    if (!filename) return null;
    
    const fullUrl = EMOJI_BASE_URL + filename;
 ...
 * @param {boolean} isMarkdown - 是否为Markdown模式
 * @returns {string} 处理后的文本
 */
export const processCoolapkEmoji = (text, isMarkdown = false) => {
    if (!text) return text;
    ...
    return text.replace(emojiPattern, (match, emojiName) => {
        ...
        // 生成img标签
        const imgTag = `<img src="${emojiUrl}" alt="[${emojiName}]" title="${emojiName}" class="coolapk-emoji" style="display: inline-block; width: 24px; height: 24px; vertical-align: middle; margin: 0 2px;" />`;
 ...
 * 用于Markdown渲染前的预处理
 */
export const processCoolapkEmojiWithCodeProtection = (text) => {
    if (!text) return text;
    ...
    // 处理表情
    processedText = processCoolapkEmoji(processedText, true);
    
    // 恢复代码块
    processedText = processedText.replace(/___CODE_BLOCK_(\d+)___/g, (match, index) => codeBlocks[parseInt(index)]);
    processedText = processedText.replace(/___INLINE_CODE_(\d+)___/g, (match, index) => inlineCodeBlocks[parseInt(index)]);
 ...
 * 检查文本是否包含酷安表情
 */
export const hasCoolapkEmoji = (text) => {
    ...
    return matches.some(match => {
        ...
        return COOLAPK_EMOJI_MAP[emojiName] !== undefined;
    });
};

export { COOLAPK_EMOJI_MAP, EMOJI_BASE_URL };